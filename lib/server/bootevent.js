...
function boot (env, language) {
  ...
  function checkNodeVersion (ctx, next) {
    ...
    if ( semver.satisfies(nodeVersion, '^12.8.0') || semver.satisfies(nodeVersion, '^10.16.0')) {
      //Latest Node 8 LTS and Latest Node 10 LTS are recommended and supported.
    ...
    else if ( semver.satisfies(nodeVersion, '^13.0.0') || semver.satisfies(nodeVersion, '^14.0.0')) {
        //Latest Node version
